...
public class ItemWoodenBucketEmpty extends ItemArmor
{
	...
	{
		return ResTool.getResource("item.woodenbucket.layer_1", ResTool.ARMORS, ModWoodenBucket.INSTANCE).toString();
	}
	...
	protected void renderBucketBlur(int par1, int par2)
	{
		...
		Minecraft.getMinecraft().getTextureManager().bindTexture(ResTool.getResource("bucketblur", ResTool.GUI, ModWoodenBucket.INSTANCE));
	...
	{
		if (par2EntityPlayer.getCurrentEquippedItem().getItem() == ModWoodenBucket.woodenBucketEmpty && !par2EntityPlayer.capabilities.isCreativeMode && par3EntityLivingBase instanceof EntityCow)
		{
			...
			ItemStack itemstack = new ItemStack(ModWoodenBucket.woodenBucketMilk);
		...
		else
		{
			...
			if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
			{
				...
				if (material == Material.water && l == 0)
				{
					...
					return this.func_150910_a(par1ItemStack, par3EntityPlayer, ModWoodenBucket.woodenBucketWater);
				}
			}
