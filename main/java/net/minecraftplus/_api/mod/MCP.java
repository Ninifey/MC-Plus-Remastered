...
public class MCP implements IMod
{
	...
	public static final String HOMEURL = "https://www.curseforge.com/minecraft/mc-mods/minecraftplus-remastered";
	...
	public void PreInitialize(FMLPreInitializationEvent parEvent)
	{
		...
		{ 	
			proxy.devTool("mcp_" ,this.modid, this.modinfo.getMap());
		}
	...
	public void Initialize(FMLInitializationEvent parEvent)
	{
		if (this.isBaseMod)
		{
			...
			MCP.ENTITIES().initialize(false);

			this.addEventHandler(FMLCommonHandler.instance().bus(), this);

			proxy.register();

			MCP.WORLDGENS().initialize(false);
			MCP.LOOTS().initialize(false);
			MCP.PACKETS().initialize(false);
			if(proxy.getSide()) {
				MCP.LANGS().initialize(false);
			}
		}
	}
	...
	public void onServerStart(FMLServerStartingEvent par1Event)
	{
		if (this.isBaseMod)
		{
			MCP.COMMANDS().initialize(proxy.getSide());
		}
	}
